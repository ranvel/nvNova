...
**`NotationController` (`nvNova/Storage/`) is the heart.** It owns the in-memory note database — `allNotes` plus a parallel C array (`allNotesBuffer`) for fast filtering — and coordinates the list data source, labels, deletion, and sync. Most subsystems hang off it.

**`NoteObject` (`nvNova/Models/`)** is the note model (`<NSCoding, SynchronizedNote>`). The "Velocity" speed comes from cached C strings (`cTitle`, `cContents`, `cLabels`) that incremental search scans directly rather than walking `NSString`s. Notes carry `nodeID` + per-disk info for file syncing and WAL storage.
